...
UINT1024 UINT1024::operator+(UINT1024 num2)
{
	...
	int currSize = 0;
	UINT1024 result = 0;
	uint64_t* pResult = (uint64_t*)&result.num;
	const uint64_t* pNum1 = (uint64_t*)(&num);
	const uint64_t* pNum2 = (uint64_t*)(&num2.num);


	for (int i = 0; i < size / 64; i++) {
		currSize = currGroupSize + 63;
		if (!*pNum1 && !*pNum2) {
			*pResult = carry;
			pResult++;
			pNum1++;
		...
		aux = (*pNum1 + *pNum2 + carry);
		*pNum1++;
        *pNum2++;

		carry = (num[currSize] && num2.num[currSize]);
...
// TODO:
// Metodo que se va a encargar de transformar un numero decimal de un formato a string a binario para meterlo en el bitset
void UINT1024::operator=(string decimal) {
...
string UINT1024::FastB2DConversion() {
	...
	do {
		...
		for (int i = iterator; i >= 0; --i) {
            DivTwoWords(r, Base32[i] , 10 , &Final[i], &r);
        }
